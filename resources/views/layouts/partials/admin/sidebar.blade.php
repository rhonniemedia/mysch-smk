  ...
  <div class="flex flex-col p-5 pb-28 gap-6 overflow-y-auto flex-1">
    <div class="flex flex-col gap-4">
      ...
      <div class="flex flex-col gap-1">
        ...
        <!-- Impor Data -->
        <a href="{{ route('import.pelajar.index') }}"
          class="{{ Request::is('import*') ? 'bg-muted' : 'bg-white hover:bg-muted' }} group w-full text-left flex items-center rounded-xl p-4 gap-3 transition-all duration-300 cursor-pointer">
          <i data-lucide="import" class="{{ Request::is('import*') ? 'text-foreground' : 'text-secondary group-hover:text-foreground' }} size-6 transition-all duration-300"></i>
          <span class="{{ Request::is('import*') ? 'font-semibold text-foreground' : 'font-medium text-secondary group-hover:text-foreground' }} transition-all duration-300">Impor Data</span>
        </a>

        <!-- Data Pelajar -->
        <a href="{{ route('admin.data') }}"
          class="{{ Request::is('data*') ? 'bg-muted' : 'bg-white hover:bg-muted' }} group w-full text-left flex items-center rounded-xl p-4 gap-3 transition-all duration-300 cursor-pointer">
          <i data-lucide="grid-2x2-check" class="{{ Request::is('data*') ? 'text-foreground' : 'text-secondary group-hover:text-foreground' }} size-6 transition-all duration-300"></i>
          <span class="{{ Request::is('data*') ? 'font-semibold text-foreground' : 'font-medium text-secondary group-hover:text-foreground' }} transition-all duration-300">Data Pelajar</span>
        </a>
