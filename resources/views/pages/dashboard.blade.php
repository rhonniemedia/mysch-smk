...
<div>
  ...
  <div class="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-8">
    <div class="lg:col-span-2 bg-gradient-to-br from-success to-emerald-700 rounded-2xl p-8 text-white flex flex-col md:flex-row items-center gap-6 shadow-xl shadow-success/20 relative overflow-hidden">
      ...
      <div class="w-24 h-28 bg-white/20 backdrop-blur-md rounded-2xl flex items-center justify-center border border-white/30 shrink-0 z-10">
        <i data-lucide="user" class="w-12 h-12 text-white"></i>
      </div>
      <div class="text-center md:text-left flex-1 z-10">
        <h2 class="text-2xl font-bold">{{ $namaUser }}</h2>
        <p class="text-emerald-100 text-sm mt-1">{{ $pelajar->kelas }} {{ $pelajar->konsKeahlian?->keahlian }} · NISN: {{ $pelajar->nisn }}</p>
        <p class="text-emerald-100 text-sm">Prog. Keahlian: {{ $pelajar->konsKeahlian?->progKeahlian?->prog_keahlian }}</p>
        <div class="mt-4 flex flex-wrap justify-center md:justify-start gap-2">
          ...
          <span class="bg-white/20 px-3 py-1 rounded-full text-[10px] font-bold tracking-widest uppercase border border-white/30">TA 2025/2026</span>
        </div>
      </div>
    ...
    <div class="flex flex-col rounded-2xl border border-border p-6 bg-white gap-4">
      ...
      <button onclick="triggerGlobalModal({type:'info',title:'Hubungi Admin WhatsApp',message:'Hubungi admin sekolah untuk pembaruan data. Tersedia Senin–Jumat pukul 07:00–15:00.',confirmText:'Mengerti'})"
  ...
  <div class="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
    <div class="flex flex-col rounded-2xl border border-border p-5 gap-3 bg-white">
      ...
      <p class="text-xs font-bold text-secondary uppercase tracking-wide">Surat Keterangan Lulus</p>
    ...
    @php
    // Cek apakah user sudah mengisi tracer study
    $isTracerFilled = $pelajar->tracerStudy ? true : false;
    @endphp
    <div class="flex flex-col rounded-2xl border border-border p-5 gap-3 bg-white">
      @if($isTracerFilled)
      <!-- Tampilan jika SUDAH diisi -->
      ...
      <span class="inline-flex items-center gap-1.5 text-sm font-bold text-success">
        <i data-lucide="check-circle" class="size-4"></i> Selesai
      </span>
      @else
      <!-- Tampilan jika BELUM diisi -->
      <div class="size-10 bg-error/10 rounded-xl flex items-center justify-center">
        <i data-lucide="file-edit" class="size-5 text-error"></i>
      </div>
      <p class="text-xs font-bold text-secondary uppercase tracking-wide">Tracer Study</p>
      <span class="inline-flex items-center gap-1.5 text-sm font-bold text-error">
        <i data-lucide="alert-circle" class="size-4"></i> Belum Diisi
      </span>
      @endif
    </div>
    <div class="flex flex-col rounded-2xl border border-border p-5 gap-3 bg-white">
      ...
      <span class="inline-flex items-center gap-1.5 text-sm font-bold text-secondary"><i data-lucide="clock" class="size-4"></i> Proses Validasi & Cetak</span>
    </div>
    <div class="flex flex-col rounded-2xl border border-border p-5 gap-3 bg-white">
      <div class="size-10 bg-primary/10 rounded-xl flex items-center justify-center"><i data-lucide="handshake" class="size-5 text-primary"></i></div>
      <p class="text-xs font-bold text-secondary uppercase tracking-wide">Perpisahan</p>
      <span class="text-sm font-bold text-foreground">Sedang dipersiapkan</span>
    </div>
  </div>
